define([
	...
	"dojo/ready",
	"dojox/mobile/common"
], function(dojo, connect, event, lang, win, domClass, domConstruct, domStyle, has, ready, dm){
/*=====
// summary:
		...
		if(changeSize){
			...
			if(parent){
				...
				if(scrollableRect.bottom >= contentBottom){ // use entire screen
					dh = screenHeight - parent.offsetTop; //
...
//>>excludeEnd("webkitMobile");
	...
	// TODO: check and not create too much cover for scrollableView
	// In dojox.app, a page may contains more than one scrollableView, 
	// so we need to create cover for each scrollableView.
	// Or we can create the cover and remove it other than undisplay when touch event happened.
